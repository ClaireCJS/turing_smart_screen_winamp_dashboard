...
class LcdComm(ABC):
    ...
    def DisplayText(
        ...
        #gagain, font is really a font path, so if it's an actual font, that's wrong;
        ...
        if use_resizing:
            ...
            resized_background = resize_with_aspect_ratio(text_image, width, height)
        ...
        if background_image:
            ...
            if  final_background.size[0] > text_layer_image.size[0] or final_background.size[1] > text_layer_image.size[1]:
                ...
                # Ensure sizes are the same - GOAT - very experimental
                if final_background.size[0] != text_layer_image.size[0] or final_background.size[1] != text_layer_image.size[1]:
...
def resize_with_aspect_ratio(image, target_width, target_height):
    ...
    #logger.debug(f"ARRRRR5:                                 (left={left}, top={top})")      #oops these don't exist yet
    ...
    logger.debug(f"RESIZE_AR5:                —— target_WxH={target_width}x{target_height}") #target_WxH=800x440
    logger.debug(f"RESIZE_AR6:                ——    new_WxH={   new_width}x{   new_height}") #target_WxH=800x440
