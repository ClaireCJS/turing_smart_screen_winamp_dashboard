#SONGS THAT CRASH THE SCREEN: C:\mp3\NoFX\2020 - NOFX vs Frank Turner - West Coast vs Wessex\07_Frank Turner - Bob (by NoFX).flac
...
device_is_upside_down = False                   #depending on where your outlet is, you might want to use the device upside down to make the cord come out in the right direction. TODO: fix bug where upside-down becomes rightside-up after power cycles
...
throttle_after_drawing_clock        = 1.00   #how long to wait, in seconds, after drawing an element of this type\___ testing theory that some device crashes are due to data overload   #.10 -> .25 ->    -> .75 -> 1
throttle_after_drawing_text         = 1.00   #how long to wait, in seconds, after drawing an element of this type\___ testing theory that some device crashes are due to data overload   #.25 -> .33 -> .5 -> .75 -> 1
throttle_after_drawing_progress_bar = 2.00   #how long to wait, in seconds, after drawing an element of this type/  #0.15 was de crashing on Alice Cooper — Billion Dollar Babies, and later in the song, so not a cover-art-processing issue. #.25 -> .50 -> 1 -> 2

####### COSMETIC/FONT CONFIG: #######
winamp_info_font_size         =  60
...
winamp_info_field_width       =  21          #width of winamp information: 21 characters wide
winamp_info_field_height      =   5          #width of winamp information:  4    rows    high
...
display_clock        = False       #and uncomment the
#isplay_clock        = True        #other line 😂
...
dashboard_logging.basicConfig(
    level=dashboard_logging.DEBUG,
...
if __name__ == "__main__":
    ...
    def initialize_frame():
        ...
        dashboard_logger.info("INIT! INIT! INIT! Initializing frame! INIT! INIT! INIT!")
        ...
        try:
            ...
            lcd_comm.Reset(sleep_time=2)                                                  # Reset screen in case it was in an unstable state (screen is also cleared) - this is the slowest part of startup
            ...
            lcd_comm.InitializeComm()                                                     # Send initialization commands
            lcd_comm.SetBrightness(level=45)   #FIRE HAZARD OVER 50!!!!                   # Set brightness in % (warning: revision A display can get hot at high brightness! Keep value at 50% max for rev. A)
            lcd_comm.SetBackplateLedColor(led_color=(255, 255, 255))                      # Set backplate RGB LED color (for supported HW only)
            #f device_is_upside_down: orientation_to_use = orientation_to_use + 2         # ^^^^^^^____adding 2 reverses it____^^^^^^^^
            if device_is_upside_down: orientation_to_use = Orientation.REVERSE_LANDSCAPE  # ^^^^^^^____adding 2 reverses it____^^^^^^^^
            #TODO moved down as experiment lcd_comm.SetOrientation(orientation_to_use)                                   # Set orientation (screen starts in Portrait)
    ...
    ############## INITIALIZE FRAME: ##############
    if not lcd_comm: lcd_comm = initialize_frame()
    ...
    force_update_title     = True                   #and our title on the first draw too
    bar_value              = 0
    ...
    en_dash = "–"
    em_dash = "—"
    if not winamp_info_monospaced_font: dash = em_dash + em_dash
    else                              : dash = em_dash
    ...
    while True:
        ...
        ################################# DISPLAY CLOCK: ################################
        dashboard_logger.debug("🕘 TIME: decide whether to render clock or not")
        if not display_clock:
            dashboard_logger.debug("🕘 TIME: ⛔ No, not rendering the clock")
        else:
            time_draw_start = time.perf_counter()
            dashboard_logger.debug("🕘 TIME: ✅ Yes, rendering the clock")
            try:
                ...
                if (formatted_time != last_formatted_time) or force_update_clock or debug_clock_tenacious:
                    ...
                    dashboard_logger.debug("🕘 TIME: done rendering")
                    time.sleep(throttle_after_drawing_clock)
                    if not display_winamp_info and not display_progress_bar: time.sleep(1)      #if this is all we're doing, sleep 1. future maintenance: may need to add other display elements here if we add them, to preserve this pause
            ...
            dashboard_logger.debug(f"🕘 TIME: ⏱ drawing time-of-day took {time_draw_end - time_draw_start}s ———") #0
        ...
        try:
            if status == cjs_winamp.PlayingStatus.Paused or status == cjs_winamp.PlayingStatus.Stopped and not cleared: #"if the track has changed"
                ...
                #do we need this?! cjs_winamp.update_rpc(w)
                pass
            ...
            #NO! time.sleep(0.5)
            #read info from WinAmp —— 0.001-0.005s
            ...
            if artist != artist_last or title != title_last:
                blurb = ""
        ...
        if display_progress_bar:
            ...
            dashboard_logger.debug(f"\t\t(drawing progress bar took {progress_bar_end - progress_bar_start} seconds)") #0.22
        #################################### DISPLAY PROGRESS BAR: END ########################################



        ####################################### DISPLAY ALBUM ART: ######################################
        #WE'RE NOT CURRENTLY DOING THIS SEPARATELY, LEAVING THIS HERE IN CASE WE CHANGE OUR MIND LATER: #dashboard_logger.debug("drawing album art")        #start = time.perf_counter()        #if force_update_cover:        #    lcd_comm.DisplayBitmap(now_playing_album_art, use_cache=False)        #    force_update_cover = False        #    force_update_title = True        #end = time.perf_counter()        #dashboard_logger.debug(f"album art (took {end - start:.3f} s)")
        dashboard_logger.debug(f"##############################################")


        ####################################### DISPLAY WINAMP INFO: ######################################
        if display_winamp_info or force_display_winamp_info:
            dashboard_logger.debug(f"WWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWWW")
            try:
                dashboard_logger.debug(f"WTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWTWT")
                if force_update_title or last_displayed_message != winamp_info_for_printing:
                    ...
                    dashboard_logger.debug(f"\n ***************** Drawing winamp info: ***************** [darken_factor_to_use={darken_factor_to_use}]")
                    ...
                    if (winamp_info_for_printing.endswith("[Paused]")): color_to_use = winamp_info_font_color_pause
                    ...
                    if background_image_path_to_use and os.path.exists(background_image_path_to_use): params["background_image"] = background_image_path_to_use                     #it literally may have disappeared in the meanwhile. we really need to use cache it when it exists 🐐
                    ...
                    dashboard_logger.debug(f"\t\t(drawing winamp info took {winamp_info_end - winamp_info_start:3f} seconds)") #1.855
            except Exception as e:
                force_frame_init = True
